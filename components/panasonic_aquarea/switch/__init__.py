...
PanasonicAquareaSwitch = panasonic_aquarea_ns.class_(
    "PanasonicAquareaSwitch", switch.Switch
)
...
def validate_switch_field(value):
    ...
    if field["type"] not in ["switch", "binary_sensor"]:
        raise cv.Invalid(
            f"Field {value['field']} type {field['type']} is not compatible with switch"
        )
