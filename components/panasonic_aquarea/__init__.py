...
HandlePacketAction = panasonic_aquarea_ns.class_(
    "HandlePacketAction", automation.Action
)
...
PanasonicAquareaProtocol = panasonic_aquarea_ns.class_(
    "PanasonicProtocolInterface", cg.Component
)
...
@PROTOCOL_REGISTRY.register(
    ...
    {
        cv.Optional(
            CONF_QUERY_INTERVAL, default="5s"
...
DATA_SOURCE_REGISTRY = Registry(
    {
        cv.GenerateID("data_source_id"): cv.declare_id(
            panasonic_aquarea_ns.class_("PanasonicAquareaDataSource")
        ),
    }
)
...
@DATA_SOURCE_REGISTRY.register(
    DATA_SOURCE_UART, PanasonicAquareaUARTDataSource, uart.UART_DEVICE_SCHEMA
)
...
def validate_data_source(value):
    ...
    if value is None:
        return cv.validate_registry_entry("data_source", DATA_SOURCE_REGISTRY)(
            {"null": {}}
        )
...
def _validate_protocol_dependencies(config):
    ...
    if "main" in protocol_names and "main_request" not in protocol_names:
        raise cv.Invalid(
            "main_request protocol is required when main protocol is active and listen_only is false"
        )
    return config
    ...
    cv.Schema(
        {
            ...
            cv.Optional(
                CONF_PROTOCOLS, default=["main", "main_request"]
            ): validate_protocols,
            cv.Optional(
                CONF_DATA_SOURCE, default=DATA_SOURCE_UART
...
async def to_code(config):
    ...
    data_source = await cg.build_registry_entry(
        DATA_SOURCE_REGISTRY, data_source_config
    )
    ...
    for conf in config.get(CONF_ON_PACKET_SEND, []):
        ...
        await automation.build_automation(
            trigger, [(cg.std_vector.template(cg.uint8), "packet")], conf
        )
...
async def handle_packet_action_to_code(config, action_id, template_arg, args):
    ...
    template_ = await cg.templatable(
        config[CONF_PACKET], args, cg.std_vector.template(cg.uint8)
    )
